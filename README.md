...
{
    "require": {
        "rodwansalman/recaptcha": "dev-master"
    }
}
```
...
1. Add `rodwansalman\Recaptcha\RecaptchaServiceProvider` to the service provider list in `app/config/app.php`.
2. Run `php artisan config:publish rodwansalman/recaptcha`.
3. In `app/config/packages/rodwansalman/recaptcha/config.php`, enter your reCAPTCHA public and private keys.
